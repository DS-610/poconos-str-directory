...
// Load .env.local FIRST, before any module imports that read process.env
...
async function main() {
  ...
  console.log('Connecting:', url.slice(0, 40));
  ...
  const { error } = await supabase.from('providers').upsert(rows, { onConflict: 'slug' });
  if (error) {
